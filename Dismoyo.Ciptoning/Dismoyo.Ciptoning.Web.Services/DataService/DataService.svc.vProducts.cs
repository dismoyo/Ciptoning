...
namespace Dismoyo.Ciptoning.Web.Services
{
    ...
    public partial class DataService : EntityFrameworkDataService<StoreFunctionsDataContext>, IServiceProvider
    {
        ...
        [QueryInterceptor("vProducts")]
        public Expression<Func<vProduct, bool>> OnQueryvProducts()
        {
            return (p => !p.IsDeleted);
        }

        [ChangeInterceptor("vProducts")]
        public void OnChangevProducts(vProduct data, UpdateOperations operations)
        {
            if (ValidateSegment("vProducts"))
            {
                var dataProvider = DataConfiguration.GetDefaultDataProvider<IvProductDataProvider>();
