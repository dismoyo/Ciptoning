...
namespace Dismoyo.Ciptoning.Web.Services
{
    ...
    public partial class DataService : EntityFrameworkDataService<StoreFunctionsDataContext>, IServiceProvider
    {
        ...
        [QueryInterceptor("vSalesOrders")]
        public Expression<Func<vSalesOrder, bool>> OnQueryvSalesOrders()
        {
            return (p => true);
        }

        [ChangeInterceptor("vSalesOrders")]
        public void OnChangevSalesOrders(vSalesOrder data, UpdateOperations operations)
        {
            var dataProvider = DataConfiguration.GetDefaultDataProvider<IvSalesOrderDataProvider>();
                    ...
                    using (var transaction = DefaultDataContext.DataContext.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            if ((prevDocumentStatusID != data.DocumentStatusID) && (data.DocumentStatusID == 2)) // Posted
                                DefaultDataContext.SalesOrderExt(data.DocumentID);
                        }
                        catch (Exception ex)
                        {
        ...
        [WebGet]
        public IQueryable<vSalesOrder> fSalesOrders()
        {
            return CurrentDataSource.fSalesOrder(
