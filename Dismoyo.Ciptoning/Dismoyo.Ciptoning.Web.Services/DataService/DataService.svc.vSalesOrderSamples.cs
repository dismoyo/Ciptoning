...
namespace Dismoyo.Ciptoning.Web.Services
{
    ...
    public partial class DataService : EntityFrameworkDataService<StoreFunctionsDataContext>, IServiceProvider
    {
        ...
        [QueryInterceptor("vSalesOrderSamples")]
        public Expression<Func<vSalesOrderSample, bool>> OnQueryvSalesOrderSamples()
        {
            return (p => true);
        }

        [ChangeInterceptor("vSalesOrderSamples")]
        public void OnChangevSalesOrderSamples(vSalesOrderSample data, UpdateOperations operations)
        {
            var dataProvider = DataConfiguration.GetDefaultDataProvider<IvSalesOrderSampleDataProvider>();
                    ...
                    using (var transaction = DefaultDataContext.DataContext.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            if ((prevDocumentStatusID != data.DocumentStatusID) && (data.DocumentStatusID == 2)) // Posted
                                DefaultDataContext.SalesOrderSampleExt(data.DocumentID);
                        }
                        catch (Exception ex)
                        {
        ...
        [WebGet]
        public IQueryable<vSalesOrderSample> fSalesOrderSamples()
        {
            return CurrentDataSource.fSalesOrderSample(
