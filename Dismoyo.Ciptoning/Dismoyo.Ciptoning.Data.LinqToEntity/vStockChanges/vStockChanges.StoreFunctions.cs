...
namespace Dismoyo.Ciptoning.Data
{
    ...
    public partial class StoreFunctionsDataContext : DataContext
    {
        ...
        #region fStockChanges

        [DbFunction("StoreFunctionsDataContext", "fStockChanges")]
        public IQueryable<vStockChanges> fStockChanges(
            Guid? p_DocumentID,
            ...
            string p_PIC,
            int? p_OldItemStatusID,
            int? p_NewItemStatusID,
            string p_ReferenceNumber,
            int? p_DocumentStatusID,
            DateTime? p_PostedDateFrom,
            DateTime? p_PostedDateTo)
        {
            return OrderQuery<vStockChanges>(((IObjectContextAdapter)this).ObjectContext
                .CreateQuery<vStockChanges>(string.Format(@"
                [{0}].[fStockChanges]
                ( 
                    @DocumentID,
                    @DocumentCode,
                ...
                DefaultDataContext.CreateParameter("OldItemStatusID", typeof(int?), p_OldItemStatusID),
                DefaultDataContext.CreateParameter("NewItemStatusID", typeof(int?), p_NewItemStatusID),
