...
(
	SELECT
            ...
            [CU].[Name] [CreatedByUserName]
        FROM
			[dbo].[UserRolePermission] [URP] INNER JOIN
            [dbo].[Role] [R] ON
            (
                ([URP].[IsUser] = 0) AND
                ([URP].[UserRoleID] = [R].[ID])
...
/****** Object:  UserDefinedFunction [dbo].[fClosingPeriod]    Script Date: 04/19/2016 11:47:57 ******/
SET ANSI_NULLS ON
GO
...
(
  SELECT [CLP].[ID]
       ...
       , [CLP].[November]
       , [CLP].[December]
       , [dbo].[ConvertLocalToUTC]([CLP].[CreatedDate]) [CreatedDate]
       , [CUSR].[ID] [CreatedByUserID]
       , [CUSR].[Name] [CreatedByUserName]
       , [dbo].[ConvertLocalToUTC]([CLP].[ModifiedDate]) [ModifiedDate]
       , [MUSR].[ID] [ModifiedByUserID]
       , [MUSR].[Name] [ModifiedByUserName]
       , [CLP].[IsDeleted]
  FROM
    [dbo].[ClosingPeriod] [CLP]
    LEFT OUTER JOIN [dbo].[User] [CUSR]
      ON ([CLP].[CreatedByUserID] = [CUSR].[ID])
    LEFT OUTER JOIN [dbo].[User] [MUSR]
      ON ([CLP].[ModifiedByUserID] = [MUSR].[ID])
    LEFT JOIN [dbo].fSite(
		@SiteID, 
		@SiteCode, 
		@SiteName,
  ...
  WHERE
    ...
    OR ([STE].[Code] = @SiteCode))
    AND ((@SiteName IS NULL)
    OR ([STE].[Name] = @SiteName))
    AND ((@Year IS NULL)
    OR ([CLP].[Year] = @Year))
    AND ((@January IS NULL)
    OR ([CLP].[January] = @January))
    AND ((@February IS NULL)
    OR ([CLP].[February] = @February))
    AND ((@March IS NULL)
    OR ([CLP].[March] = @March))
    AND ((@April IS NULL)
    OR ([CLP].[April] = @April))
    ...
    OR ([CLP].[July] = @July))
    AND ((@August IS NULL)
    OR ([CLP].[August] = @August))
    AND ((@September IS NULL)
    OR ([CLP].[September] = @September))
    AND ((@October IS NULL)
    OR ([CLP].[October] = @October))
    AND ((@November IS NULL)
    OR ([CLP].[November] = @November))
    AND ((@December IS NULL)
    OR ([CLP].[December] = @December))
    AND ((@IsDeleted IS NULL)
    OR ([CLP].[IsDeleted] = @IsDeleted))
)
...
AS SELECT [ID]
        ...
        , [IsDeleted]
   FROM
     [dbo].[fClosingPeriod](NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)
...
BEGIN
    ...
    IF NOT EXISTS(SELECT * FROM [dbo].[AutoNumberCounter] WHERE [SiteID] = @SiteID)
    BEGIN
    ...
    SELECT
            ...
            (
			    CASE
			        WHEN @TypeID = 1 THEN [SalesOrder]
			        WHEN @TypeID = 2 THEN [SalesOrderReturn]
			        WHEN @TypeID = 3 THEN [SalesOrderSwap]
			        WHEN @TypeID = 4 THEN [SalesOrderSample]
			        WHEN @TypeID = 5 THEN [SalesOrderFOC]
			        WHEN @TypeID = 6 THEN [StockOpname]
			        WHEN @TypeID = 7 THEN [StockChanges]
			        WHEN @TypeID = 8 THEN [StockTransfer]
			        WHEN @TypeID = 9 THEN [StockDisposal]
			        WHEN @TypeID = 10 THEN [DeliveryOrder]
			        WHEN @TypeID = 11 THEN [Invoice]
			        WHEN @TypeID = 12 THEN [StockReceive]
			        WHEN @TypeID = 20 THEN [Customer]
			    END + 1
		    )
        FROM
            [dbo].[AutoNumberCounter]
    ...
    UPDATE [dbo].[AutoNumberCounter]
        SET 
            [SalesOrder] = (CASE WHEN @TypeID = 1 THEN @Counter ELSE [SalesOrder] END),
            [SalesOrderReturn] = (CASE WHEN @TypeID = 2 THEN @Counter ELSE [SalesOrderReturn] END),
            [SalesOrderSwap] = (CASE WHEN @TypeID = 3 THEN @Counter ELSE [SalesOrderSwap] END),
            [SalesOrderSample] = (CASE WHEN @TypeID = 4 THEN @Counter ELSE [SalesOrderSample] END),
            [SalesOrderFOC] = (CASE WHEN @TypeID = 5 THEN @Counter ELSE [SalesOrderFOC] END),
            [StockOpname] = (CASE WHEN @TypeID = 6 THEN @Counter ELSE [StockOpname] END),
            [StockChanges] = (CASE WHEN @TypeID = 7 THEN @Counter ELSE [StockChanges] END),
            [StockTransfer] = (CASE WHEN @TypeID = 8 THEN @Counter ELSE [StockTransfer] END),
            [StockDisposal] = (CASE WHEN @TypeID = 9 THEN @Counter ELSE [StockDisposal] END),
            [DeliveryOrder] = (CASE WHEN @TypeID = 10 THEN @Counter ELSE [DeliveryOrder] END),
            [Invoice] = (CASE WHEN @TypeID = 11 THEN @Counter ELSE [Invoice] END),
            [StockReceive] = (CASE WHEN @TypeID = 12 THEN @Counter ELSE [StockReceive] END),
            [Customer] = (CASE WHEN @TypeID = 20 THEN @Counter ELSE [Customer] END)
        WHERE
            [SiteID] = @SiteID;
